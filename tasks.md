...
- [x] PostgreSQL 로컬 또는 Docker 환경 준비
- [x] PostgreSQL 연결 설정을 환경변수로 분리
- [x] SQLAlchemy 선택
- [x] `users` 테이블 설계
- [x] `health_records` 테이블 설계
- [x] `activity_records` 테이블 설계
- [x] 사용자와 건강 기록의 외래키 관계 설정
- [ ] JSON 파일 데이터를 PostgreSQL로 이전
- [x] CRUD를 PostgreSQL 기반으로 전환
- [x] 통계·검색을 PostgreSQL 쿼리로 전환
- [x] 소프트 삭제를 DB 컬럼으로 관리
- [x] Docker Compose로 API + PostgreSQL 실행
- [x] AWS 배포 환경에 PostgreSQL 연결
...
- [x] PostgreSQL 데이터 볼륨 영구 보존
- [x] AWS 보안 그룹·방화벽 점검
- [ ] HTTPS 및 도메인 연결 검토
- [x] AWS 배포 URL README 최신화
- [x] Lightsail Static IP 연결

## CI/CD 자동화

- [ ] GitHub Actions workflow 작성
- [ ] CI에서 Python 의존성 설치
- [ ] CI에서 문법 검사·API 테스트 실행
- [ ] CI에서 Docker 이미지 빌드 확인
- [ ] GitHub Secrets에 AWS 접속 정보 등록
- [ ] GitHub Secrets에 JWT·DB 비밀값 등록
- [ ] GitHub Actions에서 AWS SSH 접속
- [ ] AWS에서 `git pull` 자동 실행
- [ ] AWS에서 Docker Compose 자동 재배포
- [ ] 배포 후 헬스 체크 자동 실행
...
- [x] `GET /activity-records/summary` 기간별 활동량 요약
- [x] `GET /activity-records/daily-summary` 일별 활동량 요약
- [x] 시간대별 평균·최대 심박수 계산
- [x] 일별 걸음 수 통계
- [x] 일별 활동 에너지 통계
...
- [x] Grafana를 Docker Compose에 추가
- [x] Grafana 데이터 볼륨 설정
- [ ] PostgreSQL Grafana 읽기 전용 계정 생성
- [x] Grafana에서 PostgreSQL 데이터 소스 연결
- [ ] 사용자별 대시보드 조회 구조 설계
- [ ] 기간 선택 변수 설정
- [x] 시간별 심박수 선 그래프
- [x] 일별 걸음 수 막대 그래프
- [x] 일별 활동 에너지 그래프
- [ ] 평균·최대 심박수 요약 패널
- [x] Grafana 대시보드 저장·공유 설정
- [ ] AWS에서 Grafana 접근 포트 및 인증 설정

## 작업 진행 원칙

- [ ] 기능 구현을 먼저 묶어서 완료
- [ ] 로컬 Docker Compose 통합 테스트
- [ ] GitHub 최종 push
- [ ] AWS 수동 배포 1회 확인
- [ ] CI/CD 자동화 적용
